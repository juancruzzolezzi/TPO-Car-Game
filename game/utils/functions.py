...
def mostrar_pantalla_game_over(pantalla, fuente, puntuacion, ANCHO, ALTO, COLOR_GAME_OVER, nombre):
    ...
    pantalla.blit(encabezado, (ANCHO - 200, y_offset))  # Mover el encabezado a la derecha
    y_offset += 40

    for idx, entrada in enumerate(leaderboard[:10]):  # Limitar a los primeros 10 puntajes
        texto = fuente.render(f"{idx + 1}. {entrada['nombre']}: {entrada['puntuacion']}", True, (255, 255, 255))
        pantalla.blit(texto, (ANCHO - 200, y_offset))  # Mover el texto a la derecha
        y_offset += 30
    
    # Renderizar y dibujar el texto de la puntuación con un gap de 20 píxeles
    texto_puntuacion = fuente.render(f"{nombre}: {puntuacion} puntos", True, COLOR_TEXTO)
