...
try:
    ...
    print(
        "No se pudo cargar la imagen del auto. Asegúrate de tener un archivo'.png' en el directorio."
    )
    pygame.quit()
    sys.exit()
...
try:
    ...
    cactus_imagen = pygame.image.fromstring(
        cactus_imagen_pil.tobytes(), cactus_imagen_pil.size, cactus_imagen_pil.mode
    )
    cactus_imagen = pygame.transform.scale(cactus_imagen, (50, 68))
except pygame.error:
    print(
        "No se pudo cargar la imagen del cactus. Asegúrate de tener un archivo 'cactus.png' en el directorio 'assets/img'."
    )
    pygame.quit()
    sys.exit()
...
carretera_rect_superior = carretera_superior.get_rect(
    topleft=(MARGEN_CARRETERA, 0))

carretera_inferior = pygame.Surface((ANCHO_CARRETERA, ALTO))
carretera_inferior.fill(COLOR_CARRETERA)
carretera_rect_inferior = carretera_inferior.get_rect(
    topleft=(MARGEN_CARRETERA, -ALTO))
...
while True:
    ...
    if not pausado:
        ...
        nivel, velocidad_auto, velocidad_carretera, velocidad_obstaculo = (
            actualizar_nivel(
                distancia_recorrida,
                nivel,
        ...
        ):  # 1% de probabilidad por frame de crear un nuevo obstáculo
            crear_obstaculo(MARGEN_CARRETERA, ANCHO,
                            obstaculo_imagen, obstaculos)
        if (
            random.randint(0, 100) < 1
        ):  # 1% de probabilidad por frame de crear un nuevo obstáculo
    ...
    for y in range(
        carretera_rect_superior.y % (linea_largo + distancia_entre_lineas),
        ALTO,
        linea_largo + distancia_entre_lineas,
    ):
        pygame.draw.line(
            pantalla,
            COLOR_LINEA,
    ...
    texto_puntuacion_valor = fuente.render(
        f"{int(distancia_recorrida)} mts", True, (255, 255, 255)
    )
    pantalla.blit(texto_puntuacion, (10, 10))
    pantalla.blit(texto_nivel, (10, 60))
    pantalla.blit(texto_distancia, (10, 100))
    pantalla.blit(
        texto_puntuacion_valor, (10, 130)
    )  # Ajustar la coordenada y para que esté en el siguiente renglón
    texto_nombre_jugador = fuente.render(
        f"Jugador: {nombre_jugador}", True, (255, 255, 255)
    )  # Agrega el nombre del jugador
    if pausado:
